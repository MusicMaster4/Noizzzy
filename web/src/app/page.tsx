...
const ACCEPTED = ["audio/wav", "audio/x-wav", "audio/mpeg", "audio/mp4", "audio/aac", "audio/flac", "audio/ogg", "audio/opus", "audio/aiff", "audio/x-aiff"];
const allStages = [
...
export default function Home() {
  ...
  function validate(candidate: File) {
    ...
    if (!ACCEPTED.includes(candidate.type) && !/\.(wav|mp3|m4a|aac|flac|ogg|opus|aif|aiff|wma)$/i.test(candidate.name)) return "Unsupported format. Upload a WAV, MP3, FLAC, M4A, AAC, OGG, or AIFF file.";
    return null;
  }
  ...
  return <main className="app-shell">
    ...
    <div className="workspace" id="top">
      <header className="intro"><span className="eyebrow"><Radio size={14} /> VOICE STUDIO</span><h1>From noise to <em>presence.</em></h1><p>Drop in your audio. Noizzzy removes music and background noise, restores the voice, and delivers streaming-ready loudness.</p><div className="signal-line" aria-hidden="true">{Array.from({ length: 9 }).map((_, index) => <span key={index} />)}</div><div className="engine-notes"><div><span>01</span><strong>Mel-Band RoFormer</strong><small>Neural voice separation</small></div><div><span>02</span><strong>MossFormer2 · 48 kHz</strong><small>Full-band restoration</small></div><div><span>03</span><strong>ITU-R BS.1770</strong><small>Streaming at −16 LUFS</small></div></div></header>
      <section className="studio-card">
        ...
        {!file && <div className={`dropzone ${dragging ? "dragging" : ""}`} role="button" tabIndex={0} onClick={() => inputRef.current?.click()} onKeyDown={(event) => { if (event.key === "Enter" || event.key === " ") inputRef.current?.click(); }} onDragEnter={(event) => { event.preventDefault(); setDragging(true); }} onDragOver={(event) => event.preventDefault()} onDragLeave={() => setDragging(false)} onDrop={onDrop}>
          <input ref={inputRef} type="file" accept="audio/*,.wav,.mp3,.m4a,.aac,.flac,.ogg,.opus,.aif,.aiff,.wma" onChange={(event: ChangeEvent<HTMLInputElement>) => choose(event.target.files?.[0])} /><div className="upload-orbit"><Upload size={29} /></div><h3>Drop your audio here</h3><p>or click to choose from your computer</p><div className="format-row">{["WAV", "MP3", "FLAC", "M4A", "AAC", "OGG"].map((item) => <span key={item}>{item}</span>)}</div><small>up to 5 GB · your audio never leaves this device</small>
        </div>}
        {file && !complete && <><div className="file-card"><div className="file-icon"><FileAudio /></div><div className="file-info"><strong>{file.name}</strong><span>Audio file · {humanSize(file.size)}</span></div>{!busy && <button className="icon-button" aria-label="Remove file" onClick={reset}><X size={19} /></button>}</div>
          {!busy && <div className="automatic-flow"><div className="automatic-plan" aria-label="Automatic settings"><span><WandSparkles size={20} /></span><div><small>AUTOMATIC FLOW</small><strong>Complete streaming cleanup</strong><p>We assume background noise and apply isolation, restoration, and a −16 LUFS master.</p></div><Check size={18} /></div>{runtime && !runtime.ready && <div className="runtime-setup"><div><Gauge size={18} /><span><strong>{runtime.message}</strong><small>{runtime.installing ? `Preparing local AI · ${runtime.progress}%` : "Required once; approximately 4 GB to download."}</small></span></div><button type="button" onClick={installRuntime} disabled={runtime.installing}>{runtime.installing ? <><LoaderCircle className="spin" size={15} /> Installing</> : "Install models"}</button>{runtime.installing && <div className="runtime-progress"><span style={{ width: `${runtime.progress}%` }} /></div>}</div>}<button className="primary-action" onClick={start} disabled={Boolean(runtime && !runtime.ready)}><WandSparkles size={19} /> Clean audio for streaming <ChevronRight size={19} /></button></div>}
